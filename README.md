...
A mobile-first Three.js project built with Vite and TypeScript. Phase 2 uses a persistent Nepali Mega Mart / department-store simulator: the store is the navigable world, not a level surrounded by an open city. The current milestone combines first-person traversal with a polished cashier workstation and a lightweight register-interaction foundation.
...
- **Desktop:** Click the store once to focus the embedded game, then use WASD or arrow keys to walk. Drag the scene to look, or click once for pointer-lock mouse look. Press Escape to release the pointer. Near the staff side of the checkout, press **E** to use the register and **E** or **Escape** to exit.
- **Mobile:** Use the virtual joystick to walk and drag elsewhere on the scene to look. Near the register, use the on-screen **Use Register** and **Exit** buttons.
...
- The front contains one polished imported checkout workstation with a staged conveyor, scanner, cashier-facing register, staff mat, bagging/receipt space, and the first lightweight register interaction, plus a separate customer-service counter, carts, and two organized entrance baskets.
...
See [ASSET_MANIFEST.md](./ASSET_MANIFEST.md) for asset measurements and selection limits, [THIRD_PARTY_ASSETS.md](./THIRD_PARTY_ASSETS.md) for provenance, [CASHIER_WORKSTATION_INTERACTION.md](./CASHIER_WORKSTATION_INTERACTION.md) for the current workstation/interaction verification record, [GAME_DESIGN.md](./GAME_DESIGN.md) for direction, and [DEVELOPMENT.md](./DEVELOPMENT.md) before broader changes.
